...
// the shortcode
function contactform_shortcode($contactform_atts) {
	// attributes
	...
	// captcha
	$contactform_rand = contactform_random_number();

	// set nonce field
	$contactform_nonce_field = wp_nonce_field( 'contactform_nonce_action', 'contactform_nonce', true, false );

	// name and id of submit button
	$submit_name_id = 'contactform_send';
	...
	// processing form
	if (($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['contactform_send']) && isset( $_POST['contactform_nonce'] ) && wp_verify_nonce( $_POST['contactform_nonce'], 'contactform_nonce_action' ) ) {
		// sanitize content
		$post_data = array(
	...
	if ($sent == true) {
		contactform_redirect_success();
	} elseif ($fail == true) {
		contactform_redirect_error();
	}

	// display form or the result of submission
	if ( isset( $_GET['contactformsp'] ) ) {
		if ( $_GET['contactformsp'] == 'success' ) {
			return $anchor_begin . '<p class="contactform-info">'.esc_attr($thank_you_message).'</p>' . $anchor_end;
		} elseif ( $_GET['contactformsp'] == 'fail' ) {
