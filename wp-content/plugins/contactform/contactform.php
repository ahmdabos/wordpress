...
function contactform_init() {
	load_plugin_textdomain( 'contactform', false, dirname( plugin_basename( __FILE__ ) ) . '/translation' );
}
...
if ($list_submissions_setting == "yes") {
	...
	function contactform_custom_postype() {
		$contactform_args = array(
			'labels' => array('name' => esc_attr__( 'Submissions', 'contactform' )),
	...
	function contactform_custom_columns( $columns ) {
		$columns['name_column'] = esc_attr__( 'Name', 'contactform' );
		$columns['email_column'] = esc_attr__( 'Email', 'contactform' );
...
function contactform_action_links ( $links ) {
	$settingslink = array( '<a href="'. admin_url( 'options-general.php?page=contactform' ) .'">'. esc_attr__('Settings', 'contactform') .'</a>' );
