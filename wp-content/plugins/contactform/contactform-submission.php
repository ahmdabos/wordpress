...
if ( ! defined( 'ABSPATH' ) ) {
	exit;
		...
		if ($error == false) {
			...
			if ( $value ==  "yes" ) {
				$consent = __( 'Yes', 'contact-form' );
			} else {
				$consent = __( 'No', 'contact-form' );
			}
			// show or hide ip address
			if ($ip_address_setting == "yes") {
				$ip_address = '';
			} else {
				$ip_address = sprintf( esc_attr__( 'IP: %s', 'contact-form' ), contactform_get_the_ip() );
			}
